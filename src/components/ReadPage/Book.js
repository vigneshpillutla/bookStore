...
const applyHighlight = (ref, highlight) => {
  if (highlight.page === ref.current.getAttribute('id')) {
    const g = traverseNode(
      ref.current.contentDocument.getElementsByTagName('body')[0],
      highlight.xpath,
      0
    );
    ...
    if (g.nodeName === '#text') {
      ...
      for (let i = 0; i < parentChildrens.length; i++) {
        ...
        const l = parentChildrens[i].outerHTML
          ? parentChildrens[i].outerHTML.length
          : parentChildrens[i].length;
        index += l;
      }
...
const saveHighlight = async (ref, id, bookId, highlightColor) => {
  ...
  const res = await axios.post(highlightsPath, newHighlight, {
    ...axiosCng,
    withCredentials: true
  });
};
...
const Page = (props) => {
  ...
  useEffect(() => {
    colorRef.current = props.highlightColor;
  ...
  const addBookMark = useCallback(async () => {
    ...
    const res = await axios.post(bookMarksPath, newBookMark, {
      ...axiosCng,
      withCredentials: true
    });
  ...
  const stopHighlight = useCallback(() => {
    saveHighlight(refw, props.id, props.bookId, colorRef.current);
  }, []);
